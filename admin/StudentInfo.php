...
function archiveStudent(mysqli $conn, string $student_id, string $final_status, ?int $actor_user_id = null): bool {
    ...
    try {
        ...
        addSystemLog($conn, 'INFO', "Archived student {$data['first_name']} {$data['last_name']} ({$student_id}) as {$final_status}", 'Admin/StudentInfo.php', $actor_user_id);
...
function handleStatusChange(sel){
  ...
  if (val === 'Dropped' || val === 'Graduated'){
    ...
    document.getElementById('archiveConfirmBody').innerHTML = `
      ...
      <p style="margin:0 0 8px 0;">Once confirmed, this student’s complete record will be <b>moved to the Archive</b>. It will no longer appear on the Admin list and will be accessible only to authorized administrators.</p>
